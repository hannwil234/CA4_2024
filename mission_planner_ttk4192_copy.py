...
class turtlebot_move():
    ...
    def move_to_point(self, x, y):
        ...
        for el in NAMED_WAYPOINTS:
            if WAYPOINTS[0][0]!=x and WAYPOINTS[0][1]!=y and x==el[0] and y==el[1]:
                print("turn")
                #We must rotate to the correct angle
                print("Rotating robot to face object")
                idx = NAMED_WAYPOINTS.index([x,y,0])
                theta=THETA[idx]
                # We should adopt different parameters for different kinds of movement
                self.pid_theta.setPID(1, 0, 0)     # P control while steering
                self.pid_theta.setPoint(theta)
...
class TakePhoto:
    def __init__(self):
        ...
        # Connect image topic
        img_topic = "/camera/rgb/image_raw" #Remove "_raw" for physical
...
def making_turn_exe(end_pos_angle):
...
NAMED_WAYPOINTS = [[0.35,0.3,0],[1.85,0.25,0*pi/6],[2.9,1.2,0],[3.15,2.55,0],[4.5,0.6,0],[0.56,2.4,0],[3.65,1.55,0]]
WAYPOINTS=[]
THETA=[0,pi/2,-pi/2,0,0,pi/2,pi/2]
